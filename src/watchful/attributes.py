...
def base64str(coll):
	...
	Additional compression is done by concatenating consecutive base64 strings of the 
	same length. This compressed encoding is detected by inspecting the first 
	character in the ASCII range 35 inclusive to 42 inclusive. The rest of the string 
	should be partitioned into their own strings with length ascii_code(s[0]) - 34. 
	
	For example:
	* String: "#1234" represents "1,2,3,4"
	* String: "$1234" represents "12,34"
	* String: "&1234" represents "1234", (which is never compressed since the value 
	is smaller than the compressed)
	(compression will not be done for a base64 encoded number if it is preceded or 
	suceeded by a base64 encoded number of a different length)
	
	Here are the ascii codes for the prefixes in the examples above:
	* "#" => 35
	* "$" => 36
	* "&" => 38
	The range 35 inclusive to 42 inclusive was chosen because it contains characters 
	that do not need to be escaped in JSON, nor does the range contain comma (','), 
	which is used as a delimiter to concat all of the strings.
	"""
...
def writer(out, n_rows, n_cols):
	...
	Returns a write function that takes all of the attributes for a cell in the 
	original dataset, where a cell is a single row and column pair.
	...
	* `attr_vals` is a map from attribute name to values for the `spans`. None means 
	  that the attribute has no value for that token defined by its span.
	* `name` is an optional parameter which can be used to give a name to the spans, 
	  where the attribute value of that name is the content of the spans themselves. 
	  Examples of this is: sentences, noun_chunks, tokens or collage_names.
	"""
	...
	def write(cell_data):
		...
		# For each span data tuple for the cell with the span and attribute values 
		# for those span, do:
		for span_data in cell_data:
			...
			for (attr,vals) in attr_vals.items():
				...
				for val in vals:
					if isinstance(val, numbers.Number):
						...
						raise Exception("Attribute value must be a string, None or \
							a number. Was: " + val)
			...
			for (attr,vals) in attr_vals.items():
				...
				# Not base64 the attributes to save space since there aren't that many 
				# of them.
...
def spacy_atterize(doc):
	"""
	Extracts attributes from a SpaCy document. Attributes are associated to substrings, 
	being tokens, entities, sentences or noun chunks. Every Substring is identified by 
	its character start index and character end index.
	"""
...
def initproc(userfn, user_fn_args):
	"""
	Initializes a per-process context with the user function that will be used in 
	the imap.
...
def procrow(row):
	...
	for raw_cell in row:
		...
		# Watchful's attributes API takes byte offsets and spacy provides us with 
		# character offsets.
		byte_offsets = {}
		byte_offset = 0
...
def enrich(in_file, out_file, user_fn, *user_fn_args):
	...
	in_file: Standard csv format
	out_file: Watchful custom format for watchful client ingestion
...
def attrs_to_wf_format(in_cell, attr_names, attr_row):
	"""
	Extracts attributes from an attributes file. Attributes are associated to the 
	entire text in each input dataset cell. The entire text in each input dataset 
	cell is identified by its character start index and character end index.
	"""
	...
	for attr_name, attr_val in zip(attr_names, attr_row):
		# This is the attribute representation per input dataset cell as required 
		# by watchful client interface watchful client has a 6 uppercase characters 
		# restriction on tuple[2].
		# attr_representn = (
		# 	spans, 
		# 	{attr_name: [attr_val]}, 
...
def enrich_with_attr_file(in_file, out_file, user_fn, attr_file, attr_names=None):
	...
	arguments:
		...
		attr_names: comma delimited attribute column names to be used from in_file, 
	...
	with open(out_file, "w") as outfile:
		...
		for in_row, attr_row_all in zip(in_reader, attrs_reader):
			...
			for cell_attr_val in \
				map(lambda in_cell: user_fn(in_cell, attr_names, attr_row), in_row):
				w(cell_attr_val)


def atterize_values_in_cell(cell, name, values):
	"""
	Helper function to `create_attribute_for_values()` for finding the spans for each 
	value.
	"""
	from heapq import merge
	cell = str(cell)
	matches = [
		[(m.start(), m.end()) for m in re.finditer(pattern, cell)] for pattern in values
	]
...
def create_attribute_for_values(attribute_name, values):
	"""
	Takes an attribute name and a list of known values to create attributes for. The  
	list of values will be looked up in each cell of the currently loaded dataset. An 
	attributes file will be prepared to be loaded in watchful. Returns the filename of 
	the attribute file created, which can be used by the attributes action and 
	function: watchful.load_attributes(dataset_id, attribute_filename)
	"""
...
def get_context(attribute_filename):
	"""
	Given a filename, finds the current dataset file loaded in Watchful and produces the 
	context needed to enrich that dataset. Also returns the filename of the file used for 
	the Attributes action. See: watchful.load_attributes(dataset_id, attribute_filename)
	"""
...
def validate_attr_names(attr_names, attr_file):
	"""
	Check if all attributes names are present in the attribute file. Returns as soon as an 
	attribute name is absent, or when all attribute names match.
	"""
...
def spacy_atterize_fn(cell, nlp):
